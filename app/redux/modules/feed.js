...
export default function feed (state = initialState, action) {
  switch (action.type) {
    ...
    case ADD_NEW_DUCK_ID_TO_FEED :
      return state.merge({
        newDucksToAdd: state.get('newDucksToAdd').unshift(action.duckId)  // newDucksToAdd: [action.duckId, ...state.newDucksToAdd],
      })
    case RESET_NEW_DUCKS_AVAILABLE :
      return state.merge({
        duckIds: state.get('newDucksToAdd').concat(state.get('duckIds')),  // duckIds: [...state.newDucksToAdd, ...state.duckIds]
        newDucksToAdd: [],
        newDucksAvailable: false,
      })
    default :
